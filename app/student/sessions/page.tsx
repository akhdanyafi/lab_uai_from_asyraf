import { getStudentSessions } from '@/features/academic/actions';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all hover:shadow-md h-full flex flex-col">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isOpen ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
                                            <FileText className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h3 className="text-lg font-semibold text-gray-900 group-hover:text-[#0F4C81] transition-colors">{sess.module.title}</h3>
                                            <p className="text-sm text-gray-500">{sess.class.name}</p>
                                        </div>
                                    </div>
                                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${isOpen ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                                        {isOpen ? 'Aktif' : 'Tutup'}
                                    </span>
                                </div>
                                ...
                                <div className="mt-auto pt-4 border-t border-gray-50 space-y-2.5">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="font-medium text-gray-900">{sess.deadline.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                ...
                {sessions.length === 0 && (
                    <div className="col-span-full flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-xl bg-gray-50/50">
                        ...
                        <h3 className="text-lg font-medium text-gray-900">Belum ada sesi praktikum</h3>
                        <p className="text-sm text-gray-500 max-w-sm mt-2">
                            Anda belum memiliki sesi praktikum aktif saat ini.
                        </p>
                    </div>
                )}
            </div>
