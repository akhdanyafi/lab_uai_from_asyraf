import { getAssignmentById, submitReport } from '@/features/academic/actions';
import { getSession } from '@/lib/auth';
import { redirect, notFound } from 'next/navigation';

/**
 * Student Session Details Page (Simplified)
 * 
 * UPDATED FOR SIMPLIFIED SCHEMA:
 * - Uses getAssignmentById instead of getSessionById
 * - Reports use student.id instead of studentId
 * - Assignment has embedded title/description/filePath
 * - Class has courseName instead of course.name
 */

export default async function StudentAssignmentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // Check if student has submitted - use student.id instead of studentId
    const existingReport = assignmentData.reports.find(r => r.student.id === session.user.id);
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">{assignmentData.class.name} - {assignmentData.class.courseName}</p>
                </div>
                <div className="flex items-center gap-3">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${isOpen ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                        {isOpen ? 'Tugas Aktif' : 'Tugas Ditutup'}
                    </span>
                    <div className="text-right text-sm">
                        <p className="text-gray-400 text-xs uppercase tracking-wider font-medium">Batas Waktu</p>
                        <p className="text-gray-900 font-medium">{assignmentData.deadline.toLocaleString('id-ID', { dateStyle: 'long', timeStyle: 'short' })}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div className="prose prose-sm max-w-none text-gray-600 mb-6">
                            <p>{assignmentData.description || "Tidak ada deskripsi untuk tugas ini."}</p>
                        </div>

                        {assignmentData.filePath && (
                            <div className="border-t border-gray-50 pt-4">
                                <a
                                    href={assignmentData.filePath}
                                    target="_blank"
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-6">
                        ...
                        {existingReport ? (
                            ...
                            <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex flex-col items-center text-center">
                                ...
                                <h3 className="text-red-800 font-semibold text-sm">Tugas Ditutup</h3>
